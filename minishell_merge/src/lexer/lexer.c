...
/*   Updated: 2024/11/23 13:25:22 by fzayani          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token *parse_command_line(char *line, t_ctx *ctx)
{
    ...
    while (*line)
    {
        ...
        if (*line == '\'' && !in_double_quotes)
        {
            ...
            if (!in_single_quotes && i > 0)
            {
                buffer[i] = '\0';
                add_token(&token_list, SINGLE_QUOTE, buffer);
                i = 0;
            }
            line++;
            continue;
        ...
        if (*line == '"' && !in_single_quotes)
        {
            ...
            if (!in_double_quotes && i > 0)
            {
                buffer[i] = '\0';
                add_token(&token_list, DOUBLEQUOTE, buffer);
                i = 0;
            }
            line++;
            continue;
