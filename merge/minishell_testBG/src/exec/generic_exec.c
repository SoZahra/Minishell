		...
		int exit_code)
{
	...
	//        cmd = cmd->prev;
	//}
	clean_and_close_all_fds(ctx->current_command); // Ajouter cette ligne
	free_command_list(ctx->current_command);
	cleanup_shell(ctx);
	free_env(ctx->env_vars);
	exit(exit_code);
}
...
int	here_doc(char *delimiter, t_ctx *ctx)
{
	...
	if (WIFSIGNALED(status))  // Si le processus a été interrompu
    {
        close(pipe_fd[0]);
        return (-1);
    }
...
int	exec_builtin_once(t_ctx *ctx, t_command *cmd)
{
	...
	ctx->exit_status = execute_builtin(cmd_line, ctx);
	if (cmd->args && cmd->args[0] && ft_strncmp(cmd->args[0], "exit", 5))
		free(cmd_line);
	ret = restore_std(ctx);
	return (ret);
}
	...
	while (tmp)
	{
		if (tmp->redirs)
		{
			...
			while (tmp->redirs[i].type)
			{
				...
				{
					tmp->redirs[i].heredoc_fd = here_doc(tmp->redirs[i].file,
							get_ctx());
...
int	exec_loop(t_ctx *ctx, t_command *cmd)
{
	...
	while (tmp)
	{
		...
		if(!tmp->prev && !tmp->next && tmp->args && tmp->args[0] && is_builtin(tmp->args[0]))
		{
